...
def CaridDetect(car_pic):
    ...
    if pic_width > MAX_WIDTH:
        ...
        img = cv2.resize(
            img, (MAX_WIDTH, int(pic_hight * resize_rate)), interpolation=cv2.INTER_AREA
        )
    # 车牌识别的部分参数保存在js中，便于根据图片分辨率做调整
    f = open('D:\\py\\openCV\\VLPR\\vehicle-license-plate-recognition\\config.js')
    j = json.load(f)
    for c in j["config"]:
        if c["open"]:
            cfg = c.copy()
    ...
    for rect in car_contours:
        ...
        # 比较得到长边的斜率
        length1 = (left_point[0] - low_point[0]) ** 2 + (
            left_point[1] - low_point[1]
        ) ** 2
        length2 = (right_point[0] - low_point[0]) ** 2 + (
            right_point[1] - low_point[1]
        ) ** 2
        if length1 > length2:
        ...
        if k < 0:  # 正角度
            ...
            card_img = oldimg[
                int(low_point[1]) : int(heigth_point[1]),
...
if __name__ == '__main__':
    for pic_file in os.listdir("C:\\Users\\lirenjie\\Desktop\\test_img"):

        roi, label, color = CaridDetect(
            os.path.join(
                "C:\\Users\\lirenjie\\Desktop\\test_img",
                pic_file,
            )
        )
        write_suc = cv2.imwrite(
            os.path.join("C:\\Users\\lirenjie\\Desktop\\result", pic_file), roi
        )
        print("*" * 50)
